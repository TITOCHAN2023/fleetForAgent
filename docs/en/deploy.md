...
Agent 0.5.0 and local `fleet-tool` 0.5.0 keep the existing WSS control connection while attempting a WebRTC DataChannel. A successful direct path carries the same `{ v, type, id, corr, t, body }` used by WSS for run, panes, desktop, and plugins. ICE failure, timeout, or an old client without `rtc_v1` falls back to the existing Worker → WSS route. Remote `/mcp` and `/mcp/sse` execute inside the Worker and remain relayed because Workers do not expose a UDP socket.
