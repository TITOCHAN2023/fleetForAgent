...
| 3 原理 | 22–42s | Tool → 云端中枢 → WebSocket 扇出 Windows / Linux / macOS。设备只往外连。 |
...
| 5 上手 | 55–68 | 54 | 四样东西。网站地址、一把令牌、每台电脑装代理、把工具导入助手。中枢只跑在本机，别的电脑加不进来。要随时随地，把服务放到云上。 |
...
**画面**  
扇出：左边 Cursor / Claude，中间 `fleet.ginfo.cc`，右边 Windows · Linux · macOS（amd64 / arm64）。光点从 Tool 进中枢，再同时打到三端，结果收回来。底部小字：`设备只往外连 · 不用开端口`。
...
**口播**  
四样东西：网站地址、一把 token、每台电脑装 Agent、把 Tool 导入你的 AI。中枢只跑在 127.0.0.1，别的电脑加不进来。要随时随地，把 Server 放到云上，或直接来我们的站点。
