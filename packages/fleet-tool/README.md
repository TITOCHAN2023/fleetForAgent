...
| `read_screen` / `type` | Same optional `device_id` fill. POSIX live PTY: `read_screen` is the current VT grid (not a raw byte dump). `type` still takes `keys`; optional `key` is a named press (`enter`, `ctrl+c`). Enter is CR; `ctrl+c` is 0x03 plus SIGINT to the fg process group. |
