...
- The agent (0.2.4+) keeps one live POSIX **interactive login** shell (`-il`) started in `$HOME`, using the account login shell (passwd / `dscl` on darwin), not inherited `$SHELL`. `FLEET_SHELL` overrides the binary. After `exit`, the next `run` starts a fresh login shell in home. `cd` / `export` / aliases survive across `run`s. Windows stays one-shot `cmd /C`. fleet-tool does **not** wrap commands with `__FLEET_META__` (that fought the live shell). `wrapSessionCommand` remains as a unit-tested helper only.
