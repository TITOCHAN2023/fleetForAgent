...
- The agent (0.2.4+) keeps one live POSIX **interactive login** shell (`-il`) on a PTY (`creack/pty`, like ssh-mcp-sessions `conn.shell`) started in `$HOME`, using the account login shell (passwd / `dscl` on darwin), not inherited `$SHELL`. `FLEET_SHELL` overrides the binary. `TERM=xterm-256color` so `tty` / Codex doctor see a real terminal. Echo is disabled from the **PTY master** (termios `~ECHO`), not via slave `stty` (that becomes a SIGTTOU-stopped job and blocks `exit`). `PS1='__FLEET_PROMPT__$?\n'` — `run` writes only the user command; completion is the prompt (stripped, never returned). After `exit`, the process group is reaped and the next `run` starts a fresh login shell in home. `cd` / `export` / aliases survive across `run`s. Windows stays one-shot `cmd /C`. fleet-tool does **not** wrap commands with `__FLEET_META__`. `wrapSessionCommand` remains as a unit-tested helper only.
