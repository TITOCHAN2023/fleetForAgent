...
Only a logged-in session whose email is listed in the `ADMIN_EMAILS` secret can open `/ops` or `/v1/ops/*`. Everyone else gets 404 — the page does not exist. Empty / unset secret means no admins (forks stay closed). Comma or whitespace separated; compared case-insensitively. This is not `HUB_TOKEN` (HTTP super for list/run).
